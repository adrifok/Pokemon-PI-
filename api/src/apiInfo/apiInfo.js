...
const infoFromApi = async (byType) => {
  const api = await axios("https://pokeapi.co/api/v2/pokemon?limit=40"); //busqueda limitada a 40
  ...
  var dataBase = [...dB, ...api.results]; //voy a guardar un array de info y resultados de mi Dbase y de la api

  if (byType === "2") {//pasa solo data de la base de datos local
    dataBase = [...dB];
  } else if (byType === "1") {   //pasa solo data de la API
    dataBase = [...api.results];//si no mergea data the las dos fuentes
  }
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    if (dataBase[i].url) {//chequeo si existe una url en la API
      ...
      const pokemon = await axios(dataBase[i].url);
  ...
  const pokeById = async (id) => {//busco en la pokeApi por id
    try {   
      ...
      const pokemonById = {//creo un objeto Pokemon con sus caracteristicas
        ...
        img: "https://media.tenor.com/BjyNZZIOnbYAAAAC/pokemon.gif",
        life: api.stats[0].base_stat, //base stats means Species strengths
        force: api.stats[1].base_stat,
        defense: api.stats[2].base_stat,
        speed: api.stats[3].base_stat,
        height: api.height,
        weight: api.weight,
      };
